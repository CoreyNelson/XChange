...
public class CurrencyPair implements Comparable<CurrencyPair>, Serializable {
  ...
  public static final CurrencyPair ZEN_BTC = new CurrencyPair(Currency.ZEN, Currency.BTC);
     ...
     */
    public CurrencyPair(String string) {
        this(string, '/');
    }

    /**
     * Parses a string such as "BTC/USD" into a CurrencyPair.
     * @param string The string to parse.
     * @param delimiter The character separating the currencies. Usually '/', '_', or '-'
     */
    public CurrencyPair(String string, char delimiter) {
        try {
            String[] currencies = string.split(String.valueOf(delimiter));
            base = Currency.getInstance(currencies[0]);
            counter = Currency.getInstance(currencies[1]);
        } catch (Exception e) {
            throw new IllegalArgumentException(
                    String.format("Unable to parse CurrencyPair string '%s' with delimiter '%s'.", string, delimiter),
                    e
            );
        }
    }
